...
(function() {
  ...
  function filterComments() {
    switch (MODE) {
      case 'review': comments = allComments.filter(c => c.author_role === 'owner'); break;
      case 'premeeting': comments = allComments.filter(c => c.author === AUTHOR); break;
      case 'meeting': comments = allComments.filter(c => {
      ...
      default: comments = allComments.slice();
    }
  }
  ...
  async function deleteComment(id) { await fetch(`${API}/api/comments/${id}`, { method: 'DELETE' }); }
  ...
  function renderBubbles() {
    ...
    const grouped = {};
    comments.forEach(c => { const k = c.paragraph_index ?? -1; if (!grouped[k]) grouped[k] = []; grouped[k].push(c); });
    Object.entries(grouped).forEach(([idx, coms]) => {
      const el = annotatables[Number(idx)]; if (!el) return;
  ...
  function renderPanel() {
    ...
    if (MODE === 'meeting') title.innerHTML = '<span class="ann-live-badge">🔴 Live</span> Board Meeting';
    else title.textContent = info.title + ' (' + comments.length + ')';
    modeLabel.textContent = info.label;
    body.innerHTML = '';
    if (comments.length === 0) {
      const emptyMsg = MODE === 'premeeting' ? 'No comments from you yet. Click any paragraph to add your review notes.' : 'No comments yet';
    ...
    body.querySelectorAll('[data-action]').forEach(btn => {
      btn.onclick = async (e) => {
        ...
        if (btn.dataset.action === 'edit') { startInlineEdit(id, btn.closest('.ann-comment-card')); return; }
        else if (btn.dataset.action === 'resolve') await updateComment(id, { status: 'resolved', resolved_at: new Date().toISOString() });
        else if (btn.dataset.action === 'reopen') await updateComment(id, { status: 'open', resolved_at: null });
        else if (btn.dataset.action === 'delete') { if (confirm('Delete this comment?')) await deleteComment(id); }
        await fetchComments(); render();
      };
    });
  }
  ...
  function setupHover() {
    getAnnotatables().forEach(el => {
      ...
      el.addEventListener('click', (e) => {
        if (MODE === 'view') return;
        if (e.target.closest('.ann-inline-input') || e.target.closest('.ann-bubble') || e.target.closest('a')) return;
        e.preventDefault(); e.stopPropagation();
        const idx = getAnnotatables().indexOf(el);
        const existing = comments.filter(c => c.paragraph_index === idx);
        if (existing.length > 0) { openPanel(); scrollPanelTo(idx); }
        else openCommentInput(el);
      });
    });
  }

  function startInlineEdit(commentId, cardEl) {
    const c = allComments.find(x => String(x.id) === String(commentId)); if (!c) return;
    ...
    textEl.querySelector('.ann-edit-cancel').onclick = (e) => { e.stopPropagation(); fetchComments().then(() => render()); };
    textEl.querySelector('.ann-edit-save').onclick = async (e) => {
      e.stopPropagation(); const newText = ta.value.trim(); if (!newText) return;
      await updateComment(commentId, { comment: newText }); await fetchComments(); render();
    };
  }
  ...
  function openCommentInput(el) {
    ...
    box.innerHTML = `${selectedText ? `<div class="ann-comment-selected">"${esc(selectedText)}"</div>` : ''}<textarea placeholder="Add a comment…"></textarea><div class="ann-input-bar"><button class="ann-cancel">Cancel</button><button class="ann-submit">Comment</button></div>`;
    el.after(box); activeInput = box;
    ...
    box.querySelector('.ann-submit').onclick = async () => {
      ...
      try {
        const btn = box.querySelector('.ann-submit'); btn.textContent = 'Saving…'; btn.disabled = true;
        await postComment({ section_id: anchor.section_id, paragraph_index: anchor.paragraph_index, selected_text: selectedText, comment: text, author: AUTHOR, author_role: authorRole });
        box.remove(); activeInput = null; await fetchComments(); render(); openPanel();
      } catch(e) { console.error('Comment save failed:', e); alert('Failed to save comment — check console for details.'); }
    };
  ...
  function updateUserBadge() {
    ...
    if (MODE !== 'view' && AUTHOR) { badge.style.display = 'flex'; nameEl.textContent = AUTHOR; }
    else badge.style.display = 'none';
  }
  document.getElementById('ann-user-badge').onclick = () => {
    const name = prompt('Change your name:', AUTHOR);
    if (name && name.trim()) {
      AUTHOR = name.trim(); localStorage.setItem('board_annotation_author', AUTHOR); filterComments(); render(); updateUserBadge(); updateDraftVisibility(); if (MODE === 'review' && !isDraftUser()) setMode('view');
    }
  };
  function setMode(mode) {
    ...
    if (mode === 'meeting') { pollTimer = setInterval(async () => { await fetchComments(); render(); }, 5000); openPanel(); }
    if (mode === 'view') closePanel(); else openPanel();
    filterComments(); render(); updateUserBadge(); updateDraftVisibility();
  }
  modeBar.querySelectorAll('button[data-mode]').forEach(b => { b.onclick = () => setMode(b.dataset.mode); });
  async function init() { updateDraftVisibility(); await fetchComments(); setupHover(); render(); }
  init();
})();
